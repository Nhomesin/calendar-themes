...
router.get('/:locationId', requireLocation, asyncHandler(async (req, res) => {
  const assignments = await assignmentQueries.list(req.params.locationId);
...
router.post('/:locationId', requireLocation, asyncHandler(async (req, res) => {
  ...
  const theme = await themeQueries.get(themeId);
  if (!theme || theme.location_id !== req.params.locationId) {
    return res.status(404).json({ error: 'Theme not found' });
  }

  const assignment = await assignmentQueries.assign(
...
router.delete('/:locationId/:assignmentId', requireLocation, asyncHandler(async (req, res) => {
  await assignmentQueries.unassign(req.params.assignmentId);
  res.json({ ok: true });
}));
